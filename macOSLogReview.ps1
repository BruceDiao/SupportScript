<#
    将输出的原始日志时间格式转换为timeWindow格式，并返回一个timeWindow的字符串。
#>
...
<#
    输入原始日志文件根据条件筛选后进行对应字符串处理并返回一个数组对象。
#>
Function ConverTo-LogFile
{
    [CmdletBinding()]
    Param
    (
...
# 将原始数据按条件进行字符串处理。
    $LogFile = ConverTo-LogFile -LogPath $FilePath
...
# Windows10、11 默认Powershell版本为5.1，用于不受信任证书的运行环境。
